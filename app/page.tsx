...
export default function HomePage(){const sources = Array.from(new Set(JSON.stringify({images,referenceSolutions,projects}).match(/\/images\/[^" ]+\.jpg/g) || [])).filter(src=>existsSync(path.join(process.cwd(),'public',src)));return <MediaAvailability sources={sources}><Homepage/></MediaAvailability>}
